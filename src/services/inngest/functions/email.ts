...
export const prepareDailyUserJobListingNotifications = inngest.createFunction(
  ...
  async ({ step, event }) => {
    ...
    const [users, jobListings] = await Promise.all([
      getUsers,
      getJobListings,
    ]);

    if (jobListings.length === 0 || users.length === 0) return;
    ...
    const events = users.map(user => {
      return {
        ...
        data: {
          aiPrompt: user.aiPrompt ?? undefined,
          ...
          const filteredApplications = applications
            .filter(application => {
              return settings.find(
                setting =>
                  setting.organizationId === application.jobListing.organization.id &&
